...
const getData = async ({ coord, smp }) => {
  ...
  const { barrio = '', comuna = '' } = await fetch(getDataWsUsig(x, y)).then((r) => r.json());
  return {
    ...data,
    barrio,
    comuna,
...
const basicData = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(selectedParcel.pending, (draftState) => {
        draftState.isLoading = true;
      })
      .addCase(selectedParcel.fulfilled, (draftState, action) => {
        draftState.isLoading = false;
        draftState.isSelected = true;
        draftState.data = action.payload;
      })
      .addCase(selectedParcel.rejected, (draftState) => {
        draftState.isLoading = false;
      })
      .addCase(seekerParcel.pending, (draftState) => {
        draftState.isLoading = true;
      })
      .addCase(seekerParcel.fulfilled, (draftState, action) => {
        draftState.isLoading = false;
        draftState.isSelected = true;
        draftState.data = action.payload;
      })
      .addCase(seekerParcel.rejected, (draftState) => {
        draftState.isLoading = false;
