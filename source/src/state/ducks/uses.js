...
const uses = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(categorySelected.pending, (draftState) => {
        draftState.isLoading = true;
      ...
      .addCase(categorySelected.fulfilled, (draftState, action) => {
        if (draftState.data.length > 0) {
      ...
      .addCase(categorySelected.rejected, (draftState) => {
        draftState.isLoading = false;
      })
      .addCase(rubroSelected.pending, (draftState) => {
        draftState.isLoading = true;
        if (draftState.data.length > 0) {
          draftState.data[0].references = null;
        }
      })
      .addCase(rubroSelected.fulfilled, (draftState, action) => {
        if (draftState.data.length > 0) {
          draftState.data[0].references = action.payload;
        }
        draftState.isLoading = false;
      })
      .addCase(rubroSelected.rejected, (draftState) => {
        draftState.isLoading = false;
      });
  },
});
